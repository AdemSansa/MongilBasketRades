...
- **User decision (2026-09-10): removed `Group`'s recurring schedule fields** (`scheduleDay`/`scheduleStartTime`/`scheduleEndTime`) entirely — the academy's actual session times vary nearly every week, so the "typical weekly slot" concept never matched reality and was never wired to session creation anyway (admin always retyped the real date/time by hand). Dropped from the entity, DTOs, and the DB (explicit `ALTER TABLE ... DROP COLUMN`, since Hibernate's `ddl-auto: update` never drops columns and they were `NOT NULL`). Flutter's group-browsing screen (used during registration) now shows ages/spots/coach only. `TrainingSession.date/startTime/endTime` remains the sole source of truth for timing, unaffected.
- **Player filters + exports (2026-09-10)**, a direct admin-workflow request beyond the original MVP plan: `GET /players` gained `gender`/`groupId`/`coachId` filters (new `PlayerSpecifications`, same Specification pattern as Payments/Sessions, combinable with the existing search/status). Angular: gender + coach filter dropdowns, an **Export Excel** button (client-side `.xlsx` via the `xlsx` package, respects whatever's currently filtered — not always the full roster), and an **Export Attendance PDF** flow — pick a coach (or "All Coaches") + month + year, get a grid PDF (jsPDF + jspdf-autotable) with one column per session date, P/A/L/E per player, a rate column, one page per group. Backend: new `GET /reports/monthly-attendance` endpoint (`report` package) — this is the first thing built from `docs/api/endpoints.md`'s "Deferred to Version 2" `/reports/*` line, shipped early per direct request rather than following the original phase order. Found and fixed a real bug during testing: the PDF filename didn't include the coach, so exporting one coach's report then "All Coaches" for the same month silently overwrote the first download — fixed by slugifying the coach label into the filename. Verified end-to-end: curl (combined gender+coach filters, security boundaries), live in-browser, and the downloaded PDF was read back and visually confirmed correct against the real attendance data.
